...
namespace Cool.Benchmarks
{
    ...
    public class UncheckedAndNotByteBenchmarks
    {
        [Params(8, 200, 8192)]
        ...
        public void AndNot_Bytes()
        {
            ...
            Unchecked.AndNot(ref l, ref r, (nuint)ByteCount);
        }

        [Benchmark(Baseline = true)]
        public void Manual_AndNot_Bytes()
        {
            for (int i = 0; i < ByteCount; i++)
            {
                int idx = i + ByteOffset;
                leftBuffer![idx] &= (byte)~rightBuffer![idx];
            }
        }
    }
    ...
    [MemoryDiagnoser]
    [DisassemblyDiagnoser(maxDepth: 5, printSource: true)]
    public class UncheckedAndNotUlongBenchmarks
    {
        [Params(1, 25, 1024)]
        ...
        [Benchmark]
        public void AndNot_Ulongs()
        {
            ref ulong l = ref leftBuffer!.GetReference();
            ref ulong r = ref rightBuffer!.GetReference();
            Unchecked.AndNot(ref l, ref r, (nuint)ElementCount);
        }

        [Benchmark(Baseline = true)]
        public void Manual_AndNot_Ulongs()
        {
