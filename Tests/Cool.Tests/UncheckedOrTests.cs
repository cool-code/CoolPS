...
{
    public class UncheckedOrTests
    {
        [Fact]
        public void Or_Bytes_PerformsBitwiseOrAll()
        {
            byte[] left = new byte[] { 0x00, 0xFF, 0xAA, 0x55, 0x12, 0x34 };
            ...
            Unchecked.Or(ref left.GetReference(), ref right.GetReference(), (nuint)left.Length);

            Assert.Equal(expected, left);
        }

        [Fact]
        public void Or_UInt32_PerformsBitwiseOrAll()
        {
        ...
        [Fact]
        public void Or_Partial_OrPrefixOnly()
        {
            ...
            Unchecked.Or(ref left.GetReference(), ref right.GetReference(), (nuint)2);
        ...
        [Fact]
        public void Or_ZeroElements_NoOp()
        {
            int[] left = new int[] { 5, 6, 7 };
            int[] right = new int[] { 1, 2, 3 };
            var original = (int[])left.Clone();

            Unchecked.Or(ref left.GetReference(), ref right.GetReference(), (nuint)0);
        ...
        [InlineData(47)]
        [InlineData(63)]
        public void Or_Bytes_SlightlyLargerThanSingleVector_PerformsBitwiseOr(int len)
        {
        ...
        [InlineData(31)]
        [InlineData(65)]
        public void Or_Bytes_SoftwareFallbackEdgeCases_PerformsBitwiseOr(int len)
        {
            byte[] left = new byte[len];
            byte[] right = new byte[len];
        ...
        [Fact]
        public void Or_CustomUnmanagedStruct_PerformsFieldwiseOr()
        {
            int len = 5;
