...
public static partial class Unchecked
{
    ...
    private static void FastAnd(ref byte left, ref byte right, nuint length)
    {
        nuint offset = 0;
    ...
    public static void And(ref ulong left, ref ulong right, nuint numElements)
    {
        ...
        {
            FastAnd(ref Unsafe.As<ulong, byte>(ref left), ref Unsafe.As<ulong, byte>(ref right), length);
        }
        else
        {
    ...
    public static void And(ref uint left, ref uint right, nuint numElements)
    {
        ...
        if (Vector.IsHardwareAccelerated && length >= (nuint)Vector<byte>.Count)
        {
            FastAnd(ref Unsafe.As<uint, byte>(ref left), ref Unsafe.As<uint, byte>(ref right), length);
        }
        else
        {
            SlowAnd(ref left, ref right, length, out _, out _);
    ...
    public static void And(ref ushort left, ref ushort right, nuint numElements)
    {
        ...
        if (Vector.IsHardwareAccelerated && length >= (nuint)Vector<byte>.Count)
        {
            FastAnd(ref Unsafe.As<ushort, byte>(ref left), ref Unsafe.As<ushort, byte>(ref right), length);
        }
        else
        {
            SlowAnd(ref left, ref right, length, out _, out _);
    ...
    public static void And(ref byte left, ref byte right, nuint length)
    {
        ...
        if (Vector.IsHardwareAccelerated && length >= (nuint)Vector<byte>.Count)
        {
            FastAnd(ref left, ref right, length);
        }
        else
        {
    ...
    public static void Or(ref ulong left, ref ulong right, nuint numElements)
    {
        ...
        if (Vector.IsHardwareAccelerated && length >= (nuint)Vector<byte>.Count)
        {
            FastOr(ref Unsafe.As<ulong, byte>(ref left), ref Unsafe.As<ulong, byte>(ref right), length);
        }
        else
        {
    ...
    public static void Or(ref uint left, ref uint right, nuint numElements)
    {
        ...
        if (Vector.IsHardwareAccelerated && length >= (nuint)Vector<byte>.Count)
        {
            FastOr(ref Unsafe.As<uint, byte>(ref left), ref Unsafe.As<uint, byte>(ref right), length);
        }
        else
        {
            SlowOr(ref left, ref right, length, out _, out _);
    ...
    public static void Or(ref ushort left, ref ushort right, nuint numElements)
    {
        ...
        if (Vector.IsHardwareAccelerated && length >= (nuint)Vector<byte>.Count)
        {
            FastOr(ref Unsafe.As<ushort, byte>(ref left), ref Unsafe.As<ushort, byte>(ref right), length);
        }
        else
        {
            SlowOr(ref left, ref right, length, out _, out _);
    ...
    public static void AndNot(ref ulong left, ref ulong right, nuint numElements)
    {
        ...
        if (Vector.IsHardwareAccelerated && length >= (nuint)Vector<byte>.Count)
        {
            FastAndNot(ref Unsafe.As<ulong, byte>(ref left), ref Unsafe.As<ulong, byte>(ref right), length);
        }
        else
        {
    ...
    public static void AndNot(ref uint left, ref uint right, nuint numElements)
    {
        ...
        if (Vector.IsHardwareAccelerated && length >= (nuint)Vector<byte>.Count)
        {
            FastAndNot(ref Unsafe.As<uint, byte>(ref left), ref Unsafe.As<uint, byte>(ref right), length);
        }
        else
        {
            SlowAndNot(ref left, ref right, length, out _, out _);
    ...
    public static void AndNot(ref ushort left, ref ushort right, nuint numElements)
    {
        ...
        if (Vector.IsHardwareAccelerated && length >= (nuint)Vector<byte>.Count)
        {
            FastAndNot(ref Unsafe.As<ushort, byte>(ref left), ref Unsafe.As<ushort, byte>(ref right), length);
        }
        else
        {
            SlowAndNot(ref left, ref right, length, out _, out _);
