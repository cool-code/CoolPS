...
public static class CodePointHelper
{
    ...
    private static readonly unsafe ulong* _widePtr;
    private static readonly unsafe ulong* _ambigPtr;
    private static readonly unsafe ulong* _zeroPtr;
    private static readonly unsafe ulong* _emojiPtr;

    unsafe static CodePointHelper()
    {
        // Allocate unmanaged memory for the bitmaps to avoid the overhead of bounds checking and array indexing in safe code.
        int size = ((MaxCodePoint + 1) >> 6) * sizeof(ulong);
        _widePtr = (ulong*)Marshal.AllocHGlobal(size);
        _ambigPtr = (ulong*)Marshal.AllocHGlobal(size);
        _zeroPtr = (ulong*)Marshal.AllocHGlobal(size);
        _emojiPtr = (ulong*)Marshal.AllocHGlobal(size);

        // Initialize the bitmaps to zero to ensure all bits are cleared before filling them based on the defined ranges
        var zeroArray = new byte[size];
        Marshal.Copy(zeroArray, 0, (IntPtr)_widePtr, size);
        Marshal.Copy(zeroArray, 0, (IntPtr)_ambigPtr, size);
        Marshal.Copy(zeroArray, 0, (IntPtr)_zeroPtr, size);
        Marshal.Copy(zeroArray, 0, (IntPtr)_emojiPtr, size);

        // Fill the bitmaps based on the defined Unicode ranges for wide, ambiguous, zero-width, and emoji characters.
        FillBitmap(_wideRange, _widePtr);
        FillBitmap(_ambigRange, _ambigPtr);
        FillBitmap(_zeroRange, _zeroPtr);
        FillBitmap(_emojiRange, _emojiPtr);
    }
    ...
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static unsafe bool IsWideWidth(this CodePoint codePoint) => ((codePoint <= MaxCodePoint) && ((_widePtr[codePoint >> 6] & (1UL << (int)(codePoint & 63))) != 0)) || (((codePoint - 0x20000u) & ~0x10000u) <= 0xFFFDu);
    ...
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static unsafe bool IsAmbiguousWidth(this CodePoint codePoint) => ((codePoint <= MaxCodePoint) && ((_ambigPtr[codePoint >> 6] & (1UL << (int)(codePoint & 63))) != 0)) || (((codePoint - 0xF0000u) & ~0x10000u) <= 0xFFFDu);
    ...
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static unsafe bool IsZeroWidth(this CodePoint codePoint) => ((codePoint <= MaxCodePoint) && ((_zeroPtr[codePoint >> 6] & (1UL << (int)(codePoint & 63))) != 0)) || (codePoint - 0xE0000u <= 0xE007Fu - 0xE0000u) || (codePoint - 0xE0100u <= 0xE01EFu - 0xE0100u);

    /// <summary>
    /// Check if the code point is in the bitmap for emoji characters, which allows for O(1) checks for code points up to 0x1FFFF.
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static unsafe bool IsEmoji(this CodePoint codePoint) => (codePoint <= MaxCodePoint) && ((_emojiPtr[codePoint >> 6] & (1UL << (int)(codePoint & 63))) != 0);
}