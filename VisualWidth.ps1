...
if ($null -eq $Script:IsDetected) {
    $script:IsDetected = $false
}

function Initialize-Detection {
    if (-not $script:IsDetected) {
...
if ($null -eq $Script:RegexReady) {
    $Script:RegexReady = $false
}

function Initialize-Regex {
    if (-not $Script:RegexReady) { 
...
function Format-DirName {
    ...
    if (-not $Script:RegexReady) { Initialize-Regex }
